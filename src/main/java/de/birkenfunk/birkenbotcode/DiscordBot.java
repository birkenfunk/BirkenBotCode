    ...
    void startBot() throws LoginException {
        ...
        for (CommandDTO command : database.getCommands()) {
            if (command.isServerCommand()) {
                List<CommandDTO> commandList = StreamEx.of(shardman.retrieveCommands().complete())//
                        .map(Mapper.commandToCommandDTO)
                        .remove(it -> database.getCommands().contains(it))
                        .toList();
				StreamEx.of(commandList).forEachOrdered(it -> shardman.upsertCommand(it.getName(), it.getDescription()).addOptions().queue());
            }
        }
